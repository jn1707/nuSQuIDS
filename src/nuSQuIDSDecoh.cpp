...
void nuSQUIDSDecoh::init() {
  ...
  if ( ! ( (numneu == 2) || (numneu == 3) ) )
    std::cout << "WARNING : Code has only been tested for 2 & 3 neutrino flavors" << std::endl; 
  //TODO need to propagate my 2 flavor changes to nuSQuIDS.cpp
...
void nuSQUIDSDecoh::Set_DecoherenceGammaMatrixDiagonal(const marray<double,1>& dmat, bool standard_gell_mann) {
    ...
    //TODO Is there a more efficient way to do this? This is called at each step in the PISA fit so is called many multiple times)?
...
marray<double,2> nuSQUIDSDecoh::Get_DecoherenceGammaMatrix() const {
    ...
    */
    ...
    // auto decoherence_gamma_matrix = decoherence_gamma_matrix.GetGSLMatrix();
